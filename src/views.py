...
def spending_by_period(df, work_date: str, type_of_period: str='W') -> DataFrame:
    ...
    # Выделяем основные категории (7 позиций)
    top_7 = sorted_result.head(7)

    # Суммируем категории, не попавшие в top_7 в категорию "Остальное"
    others = pd.Series({'Остальное': sorted_result.iloc[7:].sum()})

    # Объединяем в итоговый датафрейм
    total = pd.concat([top_7, others])
...
if __name__ == '__main__':
    ...
    print(spending_by_period(df, '31.12.2021 23:59:59', 'M'))
